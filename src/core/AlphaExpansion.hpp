            ...
            for (const int node_j: model_.get_neighbors(node_i)) {
                if (node_var_ids[node_j] != -1) {
                    if (node_i < node_j) {
                        ...
                        const T e00 = model_.get_pairwise_cost(node_i, node_j, alpha_label, alpha_label);
                        const T e01 = model_.get_pairwise_cost(node_i, node_j, alpha_label, current_label_j);
